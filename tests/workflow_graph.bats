setup() {
    ...
    # re-enter the whole suite. Run from a throwaway git repo instead.
...
finish_graph_design() {
    ...
    while IFS= read -r node; do
        complete_node "$node"
    done < <(jq -r '.nodes[] | select(.type == "research") | .id' "$STATE")
    complete_node plan
}
...
@test "parallel research is a configurable fan-out joined by plan" {
    run --separate-stderr "$WRAPPER" graph init --feature research-fanout --state "$STATE" \
        --research parallel --max-parallel 2 --sequential-fallback block \
        --verify-command true
...
@test "init applies task-specific node goals atomically" {
    run "$WRAPPER" graph init --feature custom-goals --state "$STATE" \
        ...
        --node-goal implementation "Implement the purchasing recommendation" \
        --verify-command true

    [ "$status" -eq 0 ]
    [ "$(jq -r '.nodes[] | select(.id == "research") | .goal' "$STATE")" = \
        "Research inventory and order constraints" ]
    [ "$(jq -r '.nodes[] | select(.id == "implementation") | .goal' "$STATE")" = \
        "Implement the purchasing recommendation" ]
...
@test "approve-and-dispatch records the displayed revision and returns work" {
    ...
    run "$GRAPH" approve-and-dispatch --state "$STATE" --revision "$revision" \
...
@test "classic is an active bounded loop without graph approval" {
    run --separate-stderr "$WRAPPER" classic init --feature demo --state "$STATE" \
        --verify-command true --max-retries 1
...
@test "verify executes the configured command and records a real pass" {
    "$WRAPPER" classic init --feature pass --state "$STATE" \
        --verify-command "printf 'verified output\\n'" >/dev/null
    complete_node implementation

    run "$GRAPH" verify --state "$STATE" --node verification \
        --evidence "${WORK}/verification.log"
...
@test "failed verification creates a bounded fix and re-verification" {
    "$WRAPPER" classic init --feature retry --state "$STATE" \
        --verify-command "printf 'failed check\\n'; false" --max-retries 1 >/dev/null
    complete_node implementation

    run "$GRAPH" verify --state "$STATE" --node verification \
        --evidence "${WORK}/verification-failed.log"
    [ "$status" -ne 0 ]
    [ "$(jq -r '.status' "$STATE")" = "active" ]
    ...
    complete_node fix-1
    jq '.config.verification_command = "true"' "$STATE" > "${STATE}.tmp"
    mv "${STATE}.tmp" "$STATE"
    run "$GRAPH" verify --state "$STATE" --node verification-1 \
        --evidence "${WORK}/verification-pass.log"
    [ "$status" -eq 0 ]
...
@test "verification blocks the workflow when the retry budget is exhausted" {
    "$WRAPPER" classic init --feature blocked --state "$STATE" \
        --verify-command false --max-retries 0 >/dev/null
    complete_node implementation
...
@test "human gates pause a completed node until explicit approval" {
    ...
    finish_graph_design
    "$GRAPH" add-node --state "$STATE" --id legal --type review --role Reviewer \
        --goal "Review legal constraints" --order 35 --depends plan --human-gate true >/dev/null
...
@test "approval refuses a completion node that skips verification" {
    ...
    finish_graph_design
    # a committed, contributor-edited state could rewire completion straight to plan
    jq '.nodes |= map(if .id == "completion" then .dependencies = ["plan"] | .inputs = ["plan"] else . end)' \
...
@test "the retry path still reaches completed after fix and re-verification" {
    "$WRAPPER" classic init --feature retry-path --state "$STATE" \
        --verify-command "false" --max-retries 1 >/dev/null
    complete_node implementation
...
# --- Task 5: evidence survives .gitignore; reverify -------------------------

_git_target_with_impl_done() {
    # $1 = repo dir (created). $2 = feature slug (default "f"). Inits a git repo
    # + a classic workflow driven to a runnable verification node. Echoes the
    # state path.
    ...
    ( cd "$repo" && "$GRAPH" init --feature "$feat" --mode classic --state "$rstate" \
        --verify-command "printf 'ok\\n'" >/dev/null )
    printf impl > "${repo}/o/impl.txt"
    "$GRAPH" start --state "$rstate" --node implementation >/dev/null
    "$GRAPH" complete --state "$rstate" --node implementation \
        --output "${repo}/o/impl.txt" --evidence "${repo}/o/impl.txt" >/dev/null
    printf '%s\n' "$rstate"
}
...
@test "verify force-adds its evidence even under a matching .gitignore" {
    repo="$(mktemp -d)"
    printf '*.log\n' > "${repo}/.gitignore"
    rstate="$(_git_target_with_impl_done "$repo")"
    ev="${repo}/.repomethod/evidence/x.log"
    ...
    [[ "$output" == *"A  .repomethod/evidence/x.log"* ]]
    rm -rf -- "$repo"
}
...
@test "reverify regenerates evidence on a passed node" {
    repo="$(mktemp -d)"
    rstate="$(_git_target_with_impl_done "$repo")"
    "$GRAPH" verify --state "$rstate" --node verification \
        --evidence "${repo}/.repomethod/evidence/v.txt" >/dev/null
    [ "$(jq -r '.nodes[] | select(.id=="verification") | .status' "$rstate")" = "completed" ]
    [ "$(jq -r '.nodes[] | select(.id=="verification") | .outcome' "$rstate")" = "passed" ]
    nodes_before="$(jq '.nodes | length' "$rstate")"
    rm -f "${repo}/.repomethod/evidence/v.txt"
    re="${repo}/.repomethod/evidence/re.txt"
...
@test "reverify refuses a verification node that never passed" {
    ...
    [ "$status" -ne 0 ]
    [[ "$output" == *"passed verification node"* ]]
    [[ "$output" == *"status=pending"* ]]
    [ ! -e "${WORK}/re.txt" ]
}

@test "reverify refuses a verification node that failed" {
    "$WRAPPER" classic init --feature rvf --state "$STATE" \
        --verify-command "false" --max-retries 0 >/dev/null
    complete_node implementation
    run "$GRAPH" verify --state "$STATE" --node verification --evidence "${WORK}/v.log"
    [ "$status" -ne 0 ]
    run "$GRAPH" reverify --state "$STATE" --node verification --evidence "${WORK}/re.txt"
    [ "$status" -ne 0 ]
    [[ "$output" == *"passed verification node"* ]]
...
# --- T1: init pins the fork point once as config.base_ref ------------------

_base_repo() {
    # $1 = dir. main:A ; feature-base:B (out of scope) ; origin/HEAD->feature-base ;
    # work:C (in scope), no upstream. Echoes "<A-sha> <B-sha>".
    local r="$1"
    git -C "$r" init -q -b main
    git -C "$r" config user.email t@e.x
    git -C "$r" config user.name T
    git -C "$r" commit -q --allow-empty -m A
    local a b
    a="$(git -C "$r" rev-parse HEAD)"
    git -C "$r" checkout -q -b feature-base
    git -C "$r" commit -q --allow-empty -m B
    b="$(git -C "$r" rev-parse HEAD)"
    git -C "$r" symbolic-ref refs/remotes/origin/HEAD refs/heads/feature-base
    git -C "$r" checkout -q -b work
    git -C "$r" commit -q --allow-empty -m C
    printf '%s %s\n' "$a" "$b"
}

@test "init pins an explicit merge base as a 40-character SHA" {
    repo="$(mktemp -d)"
    read -r a _b < <(_base_repo "$repo")
    run bash -c "cd '$repo' && '$GRAPH' init --feature f --mode classic --state '$repo/f.json' --base main --verify-command true"
    [ "$status" -eq 0 ]
    pinned="$(jq -r '.config.base_ref' "$repo/f.json")"
    [ "$pinned" = "$a" ]
    [[ "$pinned" =~ ^[0-9a-f]{40}$ ]]
    [ "$(jq -r '.schema_version' "$repo/f.json")" = "2" ]
    rm -rf -- "$repo"
}

@test "init chooses foreign upstream then origin HEAD then main" {
    repo="$(mktemp -d)"
    read -r a b < <(_base_repo "$repo")

    # 1) foreign upstream wins
    git -C "$repo" config branch.work.remote .
    git -C "$repo" config branch.work.merge refs/heads/feature-base
    run bash -c "cd '$repo' && '$GRAPH' init --feature up --mode classic --state '$repo/up.json' --verify-command true"
    [ "$status" -eq 0 ]
    [ "$(jq -r '.config.base_ref' "$repo/up.json")" = "$b" ]

    # 2) no upstream -> origin/HEAD target
    git -C "$repo" config --unset branch.work.remote
    git -C "$repo" config --unset branch.work.merge
    run bash -c "cd '$repo' && '$GRAPH' init --feature oh --mode classic --state '$repo/oh.json' --verify-command true"
    [ "$status" -eq 0 ]
    [ "$(jq -r '.config.base_ref' "$repo/oh.json")" = "$b" ]

    # 3) no origin/HEAD -> literal main
    git -C "$repo" symbolic-ref -d refs/remotes/origin/HEAD
    run bash -c "cd '$repo' && '$GRAPH' init --feature mn --mode classic --state '$repo/mn.json' --verify-command true"
    [ "$status" -eq 0 ]
    [ "$(jq -r '.config.base_ref' "$repo/mn.json")" = "$a" ]
    rm -rf -- "$repo"
}

@test "init rejects an invalid explicit base without fallback" {
    repo="$(mktemp -d)"
    read -r _a _b < <(_base_repo "$repo")
    run bash -c "cd '$repo' && '$GRAPH' init --feature f --mode classic --state '$repo/f.json' --base no-such-ref --verify-command true"
    [ "$status" -ne 0 ]
    [[ "$output" == *"cannot resolve explicit base ref: no-such-ref"* ]]
    [ ! -e "$repo/f.json" ]
...
@test "init leaves a non-gate verification command byte-identical" {
    repo="$(mktemp -d)"
    read -r _a _b < <(_base_repo "$repo")
    run bash -c "cd '$repo' && '$GRAPH' init --feature f --mode classic --state '$repo/f.json' --verify-command 'make test'"
    [ "$status" -eq 0 ]
    [ "$(jq -r '.config.verification_command' "$repo/f.json")" = "make test" ]
    # workflow-graph.sh stores the string verbatim: it never appends --state,
    # even to a canonical-looking gate command (only the wrapper normalizes).
    run bash -c "cd '$repo' && '$GRAPH' init --feature g --mode classic --state '$repo/g.json' --verify-command '.repomethod/scripts/agent-gate.sh --spec specs/g.md'"
    [ "$status" -eq 0 ]
    [ "$(jq -r '.config.verification_command' "$repo/g.json")" = ".repomethod/scripts/agent-gate.sh --spec specs/g.md" ]
    rm -rf -- "$repo"
}

# --- T3: verification evidence defaults to a committable .txt path --------

@test "verify defaults to force-staged txt evidence" {
    repo="$(mktemp -d)"
    printf '*.log\n' > "${repo}/.gitignore"
    rstate="$(_git_target_with_impl_done "$repo" demo)"

    run "$GRAPH" verify --state "$rstate" --node verification
    [ "$status" -eq 0 ]

    ev="${repo}/.repomethod/evidence/demo-verification.txt"
    [ -s "$ev" ]
    grep -Fq "exit_code=0" "$ev"

    run git -C "$repo" status --porcelain
    [[ "$output" == *"A  .repomethod/evidence/demo-verification.txt"* ]]
    rm -rf -- "$repo"
}
...
@test "reverify reuses the default txt evidence path" {
    repo="$(mktemp -d)"
    rstate="$(_git_target_with_impl_done "$repo" demo)"
    "$GRAPH" verify --state "$rstate" --node verification >/dev/null
    ev="${repo}/.repomethod/evidence/demo-verification.txt"
    [ -s "$ev" ]
    printf 'STALE-MARKER\n' > "$ev"

    run "$GRAPH" reverify --state "$rstate" --node verification
    [ "$status" -eq 0 ]
    [ -s "$ev" ]
    grep -Fq "exit_code=0" "$ev"
    ! grep -Fq "STALE-MARKER" "$ev"

    run git -C "$repo" status --porcelain
    [[ "$output" == *"demo-verification.txt"* ]]
    [[ "$(jq -r '.nodes[] | select(.id=="verification") | .evidence[0]' "$rstate")" == *"/.repomethod/evidence/demo-verification.txt" ]]
    rm -rf -- "$repo"
}
...
@test "explicit verification evidence path still wins" {
    repo="$(mktemp -d)"
    rstate="$(_git_target_with_impl_done "$repo" demo)"
    ev="${repo}/.repomethod/evidence/custom.log"

    run "$GRAPH" verify --state "$rstate" --node verification --evidence "$ev"
    [ "$status" -eq 0 ]
    [ -s "$ev" ]
    [ ! -e "${repo}/.repomethod/evidence/demo-verification.txt" ]
    [ "$(jq -r '.nodes[] | select(.id=="verification") | .evidence[0]' "$rstate")" = "$ev" ]
    rm -rf -- "$repo"
}
